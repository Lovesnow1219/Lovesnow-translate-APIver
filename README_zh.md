...
「配音節奏」將三種設定分開：**配音語速**整片固定，預設 1.00；**英文譯文字數預算**預設每秒 2.2 詞；**整片播放倍率**則一起改變畫面與聲音。配音不再按每句長度自行提高語速。

本片的翻譯風格可以用自由文字調整；通用風格位於 `prompts/dubbing_style.md`。角色、笑點與模糊詞義由模型依本集上下文判斷，避免跨作品套用特定劇情。改風格或字數預算後，已有譯文需勾「強制重翻」才會更新。

有清晰原語言字幕的影片可勾選「以原片字幕為翻譯依據」。它會使用主審核 API 對照最多 48 張畫面，以清晰字幕校正翻譯原文（即使改梗配音與字幕不同）；整句覆寫需要至少兩張不同畫面支持，並保留逐項依據；不把作品詞彙寫成全域替換規則。修正原文後會重建相關譯文與配音，增加 API 用量。此選項預設關閉；取樣無法涵蓋整部長片，也不能保證畫面字幕或模型讀字完全正確。結果保存在本片 `source_subtitle_review.json`，可回到講者／譯文編輯確認。

「配音時間軸」頁籤列出原片／配音開始時間、逐句時長與延遲，方便回到講者／譯文編輯。過長對白仍完整播放，僅移除首尾多餘近靜音，不剪掉句尾。

原片底部已有字幕時，可在「影片輸出」把「翻譯字幕位置」選為上方。審稿 API 失敗或回傳格式錯誤會顯示「審核未完成」，不留下成功的配音後審稿戳記；成片可以預覽不代表審稿已完成。

首次升級會重建缺少快取指紋的舊配音，可能產生 API 費用；之後可重用未變動音檔。完整修正與驗證範圍請見 [審查紀錄](docs/dubbing-review-2026-09.md)。
