...
ASR_PROMPT = (
    'Transcribe every spoken word, including speech over music, opening TV or livestream voiceover, '
    'and recap. Do not skip a voice because it sits in the theme or BGM. '
    'Do not summarize, omit, clean up, or truncate. Only omit wordless music or singing with no speech.'
)
...
def _transcribe_file(wav_path, asr_model, language=None):
    ...
    last_exc = None
    try:
        from tools.target_language import asr_language_code
        lang = asr_language_code(language)
        logger.info(f'OpenAI ASR 辨識: {wav_path} model={asr_model} language={lang}')
...
def openai_transcribe_audio(wav_path, model_name: str = 'gpt-4o-transcribe-diarize', language=None, **kwargs):
    ...
    if len(chunks) <= 1:
        return _transcribe_file(wav_path, asr_model, language=language)
    ...
    try:
        for index, (start, end) in enumerate(chunks):
            ...
            segs = _transcribe_file(piece, asr_model, language=language)
            is_first = index == 0
