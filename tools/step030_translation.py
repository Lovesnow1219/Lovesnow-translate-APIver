...
def _prepare_source_lines(transcript, target_language, folder=None):
    ...
    lines = merge_utterance_cards(lines)
...
def _translate_one(summary, line, target_language, method, fixed_message, history=None, budget_scale=1.0):
    text = line['text']
    if is_asr_junk(text):
    ...
    if _is_particle_only(text):
        ...
        if ready:
            return ready, ''
    from tools.vocal_particles import particle_lead_in
    ...
    def _finish(translation, user_content=''):
        if _particle_lead and translation:
            stem = _particle_lead.rstrip('，, ')
            if not translation.lstrip().lower().startswith(stem.lower()):
    ...
    for retry in range(5):
        ...
        try:
            ...
            if str(e) != 'Invalid translation':
                time.sleep(1)
    if last_usable:
        logger.warning(f'翻譯多次未過檢查，保留無中文譯文: {last_usable[:80]}')
        return _finish(last_usable, user_content)
    ...
    if emergency:
        return _finish(emergency, user_content)
...
def _shorten_one(line, target_language, method, fixed_message, budget_scale=0.85):
    ...
    if uses_word_budget(target_language):
        ...
        extra = (
            ' Keep every clause. Speak numbers. Do not telegraph.'
            if lang in {'English', 'Vietnamese'} else ''
        )
        user_content = (
    ...
    for retry in range(5):
        ...
        try:
            ...
            success, cleaned = valid_translation(current, last_model_output, target_language, duration=duration)
            if not success:
                retry_message = cleaned
                raise Exception('Invalid translation')
            ...
            time.sleep(1)
    fallback = _usable_target_text(last_model_output, target_language)
    return fallback or current, user_content
...
def _line_needs_tighten(folder, index, line, target_language):
    translation = line.get('translation') or ''
    duration = _slot_seconds(line)
    scale = _extra_budget_scale(line)
    if uses_word_budget(target_language) and len(translation.split()) > _spoken_word_budget(duration, target_language) * scale + _word_slack(target_language):
        return True, 0.85
    if uses_char_budget(target_language) and len(translation) > _spoken_char_budget(duration, target_language) * scale + _char_slack(target_language):
        return True, 0.85
    wav_sec = _tts_wav_seconds(folder, index)
    if wav_sec is not None and wav_sec > duration * 1.12 and wav_sec > duration + 0.15:
        return True, 0.8
    return False, 1.0


def tighten_overlong_lines(folder, target_language, method='OpenAI'):
    from tools.target_language import required_translation_version, save_dub_meta
    ...
    for i, line in enumerate(transcript):
        need, scale = _line_needs_tighten(folder, i, line, target_language)
        if not need:
            continue
    ...
    save_dub_meta(folder, translation=target_language, translation_version=required_translation_version(target_language))
...
def refresh_translation_lines(folder, target_language, method='OpenAI'):
    ...
    from tools.target_language import clear_tts_cache, required_translation_version, save_dub_meta
...
def fill_changed_source_translations(folder, target_language, method='OpenAI'):
    ...
    if not changed:
        return 0
    with open(trans_path, 'w', encoding='utf-8') as handle:
...
def _translate(summary, transcript, target_language='简体中文', method='LLM'):
    ...
    if method in ('OpenAI', '阿里云-通义千问'):
        if method == 'OpenAI':
            workers, n_keys = workers_for_keys(
                'TRANSLATE_WORKERS', 'OPENAI_LUNA_API_KEY', 'OPENAI_API_KEY', default_per_key=8
            )
        else:
...
def translate(method, folder, target_language='简体中文'):
    ...
    if os.path.exists(translation_path):
        logger.info(f'目標語言改為 {target_language}，清除舊譯文後重翻')
        clear_translation_cache(folder)
