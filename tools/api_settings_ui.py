...
def build_api_settings_ui():
    with gr.Blocks() as demo:
        gr.Markdown(
            '## API 設定\n'
            '填入金鑰後按儲存，寫入專案根目錄的 `.env`（已加入 `.gitignore`，開源時不會被提交）。\n'
        ...
        save_btn.click(save_from_ui, inputs=boxes, outputs=[result, status, extra_status, *boxes])
        refresh_btn.click(load_from_ui, outputs=[status, extra_status, *boxes])
        demo.load(load_from_ui, outputs=[status, extra_status, *boxes])
